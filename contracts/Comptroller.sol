    ...
    Exponential
{
    ...
    function liquidateBorrowAllowed(
        ...
        (MathError mathErr, uint maxClose) = mulScalarTruncate(
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            if (
                isEasyMode[account] &&
                !markets[address(asset)].isEasyModeSupport
            ) {
                // Skip
                continue;
            } else {
                ...
                if (isEasyMode[account]) {
                    vars.collateralFactor = Exp({
                        mantissa: markets[address(asset)]
                            .collateralFactorMantissaEasyMode
                    });
                } else {
                    vars.collateralFactor = Exp({
                        mantissa: markets[address(asset)]
    ...
    function _setCollateralFactorEasyMode(
        CToken cToken,
        uint newCollateralFactorMantissaEasyMode
    ) external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return
        ...
        Exp memory newCollateralFactorExp = Exp({
            mantissa: newCollateralFactorMantissaEasyMode
        });
        ...
        // If collateral factor != 0, fail if price == 0
        if (
            newCollateralFactorMantissaEasyMode != 0 &&
            oracle.getUnderlyingPrice(cToken) == 0
        ) {
            return
                fail(
        ...
        // Set market's collateral factor to new collateral factor, remember old value
        uint oldCollateralFactorMantissaEasyMode = market
            .collateralFactorMantissaEasyMode;
        market
            .collateralFactorMantissaEasyMode = newCollateralFactorMantissaEasyMode;

        // Emit event with asset, old collateral factor, and new collateral factor
        emit NewCollateralFactor(
            cToken,
            oldCollateralFactorMantissaEasyMode,
            newCollateralFactorMantissaEasyMode
        );
    ...
    function _supportMarket(CToken cToken) external returns (uint) {
        ...
        markets[address(cToken)] = Market({
            isListed: true,
            isComped: false,
            isEasyModeSupport: false,
            collateralFactorMantissa: 0,
            collateralFactorMantissaEasyMode: 0
        });
    ...
    function refreshCompSpeedsInternal() internal {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            uint newSpeed = totalUtility.mantissa > 0
    ...
    function _setEasyModeSupport(
        ...
        // Set
        market.isEasyModeSupport = state;

        return uint(Error.NO_ERROR);
    }

    function _setEasyMode(bool state) public returns (bool) {
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            if (
                ...
                revert("borrowBalance not zero");
            }
        }

        isEasyMode[account] = state;
        return state;
    }
}
